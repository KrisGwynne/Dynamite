class Bot {
    makeMove(gamestate) {
        ...
        if (p1Moves.lastMove === 'D') {                                 //This is maybe not a good idea
            move = 'W';
        } else if (dynamiteDiff >= 0 && p2Moves.lastMove !== 'D') {     //If the opponent has used more dynamite than us we should use some
            move = 'D';
        } else if (1000 - p1Moves.totalMoves < 100 && p1Moves.dynamite < 100) {     //If we are running out of time to use dynamite we shoudl use it
            move = 'D';
        } else if (isP2Random) {
